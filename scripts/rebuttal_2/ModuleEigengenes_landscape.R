...
i_plts <- lapply(
    ...
    function(i) {
        ...
        expr_plt <- ggplot() +
            ...
            geom_text(
                data = i_i_plt_dat %>%
                    distinct(ME, Module, label_colour, MaxGeneCorrelation),
                mapping = aes(
        ...
        combined_plt <- cowplot::plot_grid(
            plotlist = list(expr_plt_tbl, annot_plt), ncol = 2, align = "h",
...
#     }
# )
ggsave(file="data/figures/rebuttal_2/ModuleEigengenes.pdf", device = 'pdf', plot = plots, width = 297, height = 210, units = "mm", dpi = 72)
