...
gM1_edges <- readxl::read_xlsx(
    "data/ppi/GenePPIAnnotated.xlsx", sheet = "gM1"
) %>%
    filter()
iM1_edges <- readxl::read_xlsx(
    "data/ppi/GenePPIAnnotated.xlsx", sheet = "iM1"
)
iM30_edges <- readxl::read_xlsx(
    "data/ppi/GenePPIAnnotated.xlsx", sheet = "iM30"
)
vertices <- readxl::read_xlsx(
    "data/ppi/GenePPIAnnotated.xlsx", sheet = "Vertices", 
    col_types = "text"
)
...
iM30_components_enr <- iM30_components %>%
    setNames(
        nm = sapply(., function(component) {
            vertex_attr(component, name = "Transcript Symbol")[[1]]
        })
